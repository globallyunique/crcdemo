 ...
 * @group file_url
 */
class FileUrlWidgetTest extends WebDriverTestBase {
   ...
   * Tests the file URL widget.
   */
  public function testFileUrlWidget(): void {
    // Add a new entity.
    ...
    $this->assertSession()->waitForText("The URL $url5 is not valid.");
  }
   ...
   */
  protected function addFileUrlItem(string $field_name, string $file_mode, $value, bool $check_presence = TRUE): void {
    ...
    if ($file_mode === 'upload') {
      ...
      $link = $this->files[$value]->filename;
    }
    elseif ($file_mode === 'remote') {
      $wrapper->fillField('Remote URL', $value);
      $link = $value;
    }
    // Check that the URL has been added to the field item list.
    if ($check_presence && !$this->assertSession()->waitForLink($link)) {
      $this->fail("Link '$link' not found");
    }
  }
   ...
   *   If the field doesn't exist.
   */
  protected function removeFileUrlItem(string $field_name, int $delta = 0): void {
    $session = $this->getSession();
    $page = $session->getPage();
   ...
   */
  protected function assertOrderInPageText(array $items): void {
    $session = $this->getSession();
    $text = $session->getPage()->getText();
    $strings = $not_found = [];
    foreach ($items as $item) {
      if (!$this->assertSession()->waitForText($item) || ($pos = strpos($text, $item)) === FALSE) {
