...
class FileUrlWidget extends FileWidget {
  ...
  public static function process($element, FormStateInterface $form_state, $form) {
    $file_url_type = $element['#value']['file_url_type'] ?? NULL;
    $file_url_remote = (string) $element['#value']['file_url_remote'];
  ...
  public static function validate(array &$element, FormStateInterface $form_state, array &$complete_form) {
    ...
    $file_url_types = [static::TYPE_UPLOAD, static::TYPE_REMOTE];
    if (($remote_url || $fids) && !in_array($file_url_type, $file_url_types, TRUE)) {
      // @todo Guess the type from values. Temporary disable this validation.
      // $form_state->setError($element, $this->t(
      // "A type should be selected. Either 'Upload file' or 'Remote file URL'."
      // ));
    }
