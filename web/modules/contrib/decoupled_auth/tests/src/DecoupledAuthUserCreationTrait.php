...
namespace Drupal\Tests\decoupled_auth;
...
trait DecoupledAuthUserCreationTrait {
  ...
  protected function createDecoupledUser($email_prefix = NULL) {
    ...
    $user->save();
    $this->assertNotNull($user, new FormattableMarkup('Decoupled user successfully created with the email %email.', ['%email' => $user->getEmail()]));
    return $user;
  }
