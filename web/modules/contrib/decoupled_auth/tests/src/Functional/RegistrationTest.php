...
class RegistrationTest extends BrowserTestBase {
  ...
  public function testNormalNone() {
    ...
    if (empty($accounts)) {
      ...
      $this->assertEquals($edit['name'], $account->getAccountName());
    }
  }
  ...
  public function testNormalSingle() {
    ...
    if (empty($accounts)) {
      ...
      $this->assertEquals($edit['name'], $account->getAccountName());
      $this->assertNotEquals($user_1->id(), $account->id());
    }
    ...
    $this->assertSession()->pageTextContains('The email address ' . $email_prefix . '@example.com is already taken.');
  }
  ...
  public function testAcquisitionSingle() {
    ...
    if (empty($accounts)) {
      ...
      $this->assertEquals($email, $account->getEmail());
      $this->assertEquals($edit['name'], $account->getAccountName());
      $this->assertEquals($user_1->id(), $account->id());
    }
  ...
  public function testNormalMultiple() {
    ...
    if (empty($accounts)) {
      ...
      $this->assertEquals($email, $account->getEmail());
      $this->assertNotEquals($user_1->id(), $account->id());
      $this->assertNotEquals($user_2->id(), $account->id());
    }

    // Test registering a new user when one existing user is coupled.
    // Expected result: fail with validation error.
    $this->registerNewUser('', $user_1->email_prefix);
    $this->assertSession()->pageTextContains('The email address ' . $user_1->email_prefix . '@example.com is already taken.');
  }
