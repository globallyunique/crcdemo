...
class UserProfileFieldTest extends KernelTestBase {
   ...
   * @var array
   */
  protected static $modules = ['decoupled_auth', 'field', 'user', 'system'];
  ...
  public function testEntityQuerySingleSort() {
    ...
    $result = $query->execute();
    $this->assertEquals(5, count($result), 'Five users found.');

    // Get the test field values out of the result to check the sort.
    $result_values = [];
    foreach (User::loadMultiple($result) as $user) {
      $result_values[] = $user->profile_test_single->entity->test_field->value;
    }

    $sorted_copy = $result_values;
    usort($sorted_copy, function ($first, $second) {
      return strcasecmp($first, $second);
    });

    $this->assertEquals($result_values, $sorted_copy);
  }
  ...
  public function testEntityQueryMultipleSort() {
    ...
    foreach ($users as $user) {
      ...
      foreach ($user->profile_test_multiple as $profile_item) {
        if (isset($value)) {
          // Get the smallest of the 2. Note that these are not integers,
          // they're numeric strings so don't do a > comparison. Use strcasecmp
          // instead and take the smaller one.
          $value = strcasecmp($value, $profile_item->entity->test_field->value) > 0
            ? $profile_item->entity->test_field->value
            : $value;
        }
        else {
          $value = $profile_item->entity->test_field->value;
        }
      }
      $result_values[] = $value;
    }
  ...
  public function testEntityQueryMultipleSortWithDelta() {
    ...
    $result_values = [];
    foreach (User::loadMultiple($result) as $user) {
      $result_values[] = $user->profile_test_multiple->get(1)->entity->test_field->value;
    }
