...
class AcquisitionService implements AcquisitionServiceInterface {
  ...
  /**
   * The default context for the acquisition process.
   *
  ...
  public function acquire(array $values, array $context = [], &$method = NULL) {
    ...
    // Look for a match.
    if (!empty($values)) {
      $user = $this->findMatch($values, $context);
    }
    ...
    if (!$user && $context['behavior'] & self::BEHAVIOR_PREFER_COUPLED) {
    ...
    elseif ($context['behavior'] & self::BEHAVIOR_CREATE) {
      $method = 'create';
