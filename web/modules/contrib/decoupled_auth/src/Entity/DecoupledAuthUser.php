...
class DecoupledAuthUser extends User implements DecoupledAuthUserInterface {
  ...
  public static function postLoad(EntityStorageInterface $storage, array &$entities) {
    /** @var DecoupledAuthUser[] $entities */
  ...
  public function updateProfileFields(array $types, $save = TRUE) {
    // Don't do anything if this is the anonymous user.
    if ($this->isAnonymous()) {
      return;
    }
    ...
    foreach ($types as $type) {
      ...
      $field_name = 'profile_' . $type;
      if (!$this->hasField($field_name)) {
        continue;
      }

      // Check that field values have changed.
      $current = $this->{$field_name}->getValue();
      $this->{$field_name} = $profile_storage->loadMultipleByUser($this, $type, TRUE);
      $new = $this->{$field_name}->getValue();
