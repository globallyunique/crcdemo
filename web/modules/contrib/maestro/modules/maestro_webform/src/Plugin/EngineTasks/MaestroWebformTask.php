...
class MaestroWebformTask extends PluginBase implements MaestroEngineTaskInterface {
  ...
  public function getExecutableForm($modal, MaestroExecuteInteractive $parent) {
    ...
    $maestro_config = \Drupal::config('maestro.settings');
    ...
    if ($webform_submission) {
      \Drupal::logger('maestro:webform')->info('webform_submission exists');
      ...
      if (isset($templateTask['data']['show_edit_form']) && $templateTask['data']['show_edit_form'] == 1) {
        //Detect if we're using a sitewide token.  If so, we'll use that to redirect to the webform.
        $sitewideToken = $maestro_config->get('maestro_sitewide_token');
        ...
        // MV begin
        // Added patientID to URL so that the patient_card block can be displayed when editing a webform.
        // Added patientID always under the assumption that there is no harm having it as an extra parameter in the URL.
        // Added to all places even though some of these paths may never occur for the siteHub workflows.
        $patient_id = MaestroEngine::getProcessVariable('patient_node_id', $this->processID); //MV added
        if($sitewideToken != '') {  //Use a queue Token in the URL.  Webform Form Alter supports the sitewide token as a query parameter
          $queueToken = MaestroEngine::getTokenFromQueueId($this->queueID);
          if($ui_toolbar) {
            $url = Url::fromUserInput('/admin/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => [$sitewideToken => $queueToken, 'patientID' => $patient_id]]); // MV added patientID
          }
          else {
            $url = Url::fromUserInput('/admin/structure/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => [ $sitewideToken => $queueToken, 'patientID' => $patient_id]]); // MV added patientID
          }
        }
        else {  //query parameter will remain as maestro=1&queueid=...
          if($ui_toolbar) {
            $url = Url::fromUserInput('/admin/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => ['maestro' => 1, 'queueid' => $this->queueID, 'patientID' => $patient_id]]); // MV added patientID
          }
          else {
            $url = Url::fromUserInput('/admin/structure/webform/manage/' . $webformMachineName . '/submission/' . $sid . '/edit', ['query' => ['maestro' => 1, 'queueid' => $this->queueID, 'patientID' => $patient_id]]); // MV added patientID
          }
        }
        // MV end
        $response = new RedirectResponse($url->toString());
        \Drupal::logger('maestro:webform')->info('returning from has submission path with response redirecting webform_submission to:' . $url->toString());
    ...
    // No submission entity exists, but we DO we have a submission ID?
      ...
      // No submission, so redirect to the webform creation.
        ...
        if ($webformNode && $webformNode !== FALSE) {
          $url = Url::fromUserInput('/' . $webformNode, ['query' => ['maestro' => 1, 'queueid' => $this->queueID, 'patientID' => $patient_id]]);
          \Drupal::logger('maestro:webform')->info('webformNode:' . $webformNode);
        }
        else {
          // MV: This is the URL that is used when the webform is not attached to a node so that it includes the patientID.
          // Need to get the patientID onto the URL so the patient_card block can be displayed.
          $patient_id = MaestroEngine::getProcessVariable('patient_node_id', $this->processID);
          if (isset($patient_id) && $patient_id > 0) {
            $url = Url::fromUserInput('/webform/' . $webformMachineName, ['query' => ['maestro' => 1, 'queueid' => $this->queueID, 'patientID' => $patient_id]]);
            \Drupal::logger('maestro:webform')->info('webformMachineName:' . $webformMachineName. ' patient_id:' . $patient_id);
          }
          else {
            $url = Url::fromUserInput('/webform/' . $webformMachineName, ['query' => ['maestro' => 1, 'queueid' => $this->queueID]]);
            \Drupal::logger('maestro:webform')->info('webformMachineName:' . $webformMachineName. ' patient_id not set');
          }
          // MV end
        }

        $response = new RedirectResponse($url->toString());
        \Drupal::logger('maestro:webform')->info('returning from no submission path returning response redirecting to No submission entity exists to:' . $url->toString());
        //\Drupal\utility_functions\UtilityFunctions::orchestrate(); // MV
        return $response;
      }
    }
  ...
  public function handleExecuteSubmit(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::logger('maestro:handleExecuteSubmit')->info('queueID:' . $queueID . ' triggeringElement:' . $triggeringElement['#id']);
    if (strstr($triggeringElement['#id'], 'edit-submit') !== FALSE && $queueID > 0) {
      ...
      //MV added logger call
      \Drupal::logger('maestro:handleExecuteSubmit')->info('calling maestro_webform_submission_set_cancel_completion_status');
